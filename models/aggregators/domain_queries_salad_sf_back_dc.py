...
class DomainQueriesSALADSFBackDC(nn.Module):
    """
    ...
    def __init__(self,
        ...
        if divide > 1:
            ...
            self.queries_feature_list = nn.ModuleList([QuerySelfAttn(self.cluster_dim, self.divide_query_list[i], nheads=self.cluster_dim // 32, self_attn=self_attn) if self.divide_query_list[i] > 0 else None for i in range(len(self.divide_query_list))])
    ...
    def forward(self, x, domain_idx=None):
        ...
        if self.divide > 1:
            ...
            if domain_idx is None:
                f = [self.queries_feature_list[i]().permute(0, 2, 1).repeat(x.shape[0], 1, 1) if self.divide_query_list[i] > 0 else None for i in range(len(self.divide_query_list))]
                f = torch.cat([f[i] for i in range(len(f)) if f[i] is not None], dim=2)
                ...
                if self.divide_query_list[i] > 0:
                    ...
                    for j in torch.unique(domain_idx):
                        ...
                        if i == j:
                            ...
                            domain_q_f = q_f.detach().repeat(bs, 1, 1)
