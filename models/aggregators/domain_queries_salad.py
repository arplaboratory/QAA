...
class DomainQueriesSALAD(nn.Module):
    ...
    def forward(self, x, domain_idx=None):
        ...
        if self.divide > 1:
            ...
            if domain_idx is None:
                ...
                p = torch.cat([self.score_list[i](x, self.queries_list[i]())[0] for i in range(self.divide)], dim=1) # For each domain
                if self.shared_clusters > 0:
            ...
            raise NotImplementedError("Coupled score network is not implemented yet")
    ...
    def generate_score_from_decoupled_pnet(self, x, q, domain_idx):
        if self.padding == "none":
            p = torch.cat([self.score_list[i](x[domain_idx == i], q[i]())[0] for i in range(self.divide)], dim=0)
